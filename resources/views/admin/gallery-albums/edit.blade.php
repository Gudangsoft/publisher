            ...
            <div class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" 
                              class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                              placeholder="Deskripsi singkat album...">{{ old('description', $album->description) }}</textarea>
                    @error('description')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <!-- Cover Image -->
                <div x-data="{ fileName: '', preview: null, removeCover: false }">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Cover Album</label>
                    
                    @if($album->cover_image)
                    <div class="mb-3 relative inline-block" x-show="!removeCover">
                        <img src="{{ asset('storage/' . $album->cover_image) }}" alt="Current cover" class="h-32 rounded-lg object-cover border">
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-primary-400 transition-colors">
                        ...
                        <label for="cover-file" class="cursor-pointer">
                            ...
                            <p class="text-sm text-gray-600 font-medium" x-text="fileName || 'Klik untuk ganti gambar cover'"></p>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" id="display_order" name="display_order" value="{{ old('display_order', $album->display_order) }}" min="0"
                ...
                <div class="bg-gray-50 rounded-xl p-4 border border-gray-100">
                    <h3 class="text-sm font-semibold text-gray-700 mb-2">Info Album</h3>
                    <div class="grid grid-cols-3 gap-4 text-center">
                        <div>
                            <p class="text-2xl font-bold text-primary-600">{{ $album->galleries()->count() }}</p>
                            <p class="text-xs text-gray-500">Total Foto</p>
                        </div>
                        <div>
                            <p class="text-2xl font-bold text-green-600">{{ $album->galleries()->where('is_active', true)->count() }}</p>
                            <p class="text-xs text-gray-500">Aktif</p>
                        </div>
                        <div>
                            <p class="text-2xl font-bold text-gray-600">{{ $album->created_at->format('d M Y') }}</p>
            ...
            <div class="p-6">
                <div class="border-2 border-dashed rounded-xl p-6 text-center transition-all duration-300"
                    ...
                    <label for="photos-input" class="cursor-pointer">
                        <div class="flex flex-col items-center">
                            ...
                            <p class="text-xs text-gray-500 mt-1">Foto baru akan ditambahkan ke album ini</p>
                        </div>
                    </label>
                </div>
                ...
                <div x-show="previews.length > 0" class="mt-4">
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-sm font-semibold text-gray-700"><span x-text="previews.length"></span> foto baru dipilih</p>
