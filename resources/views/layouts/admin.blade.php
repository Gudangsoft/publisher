...
<body class="bg-gray-50 font-sans antialiased">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0 lg:w-20'" 
        ...
        <div class="h-full px-3 pb-4 overflow-y-auto bg-white">
            <ul class="space-y-2 font-medium">
                ...
                @if(auth()->user()->hasPermission('book-loans'))
                <li>
                    <a href="{{ route('admin.book-loans.index') }}" class="flex items-center p-3 text-gray-900 rounded-lg hover:bg-primary-50 group {{ request()->routeIs('admin.book-loans.*') ? 'bg-primary-50 text-primary-600' : '' }}">
                        <svg class="w-5 h-5 text-gray-500 transition duration-75 group-hover:text-primary-600 {{ request()->routeIs('admin.book-loans.*') ? 'text-primary-600' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
                        </svg>
                        <span :class="sidebarOpen ? 'ml-3' : 'lg:hidden ml-3'">Data Peminjaman</span>
                    </a>
                </li>
                @endif

                @if(auth()->user()->hasPermission('visitor-logs'))
                <li>
                    <a href="{{ route('admin.visitor-logs.index') }}" class="flex items-center p-3 text-gray-900 rounded-lg hover:bg-primary-50 group {{ request()->routeIs('admin.visitor-logs.*') ? 'bg-primary-50 text-primary-600' : '' }}">
                        <svg class="w-5 h-5 text-gray-500 transition duration-75 group-hover:text-primary-600 {{ request()->routeIs('admin.visitor-logs.*') ? 'text-primary-600' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/>
                        </svg>
                        <span :class="sidebarOpen ? 'ml-3' : 'lg:hidden ml-3'">Data Pengunjung</span>
                    </a>
                </li>
                @endif
    ...
    <div :class="sidebarOpen ? 'lg:ml-64' : 'lg:ml-20'" class="sidebar-transition p-4 pt-24">
        ...
        @if(session('admin_user_id'))
            <div class="mb-4 p-4 bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-xl shadow-sm">
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        <svg class="w-5 h-5 mr-3" fill="currentColor" viewBox="0 0 20 20">
